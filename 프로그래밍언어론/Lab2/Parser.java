...
*/
    ...
    // <readStmt> -> read id;
    	match(Token.READ); // 현재 토큰이 read가 맞는지 확인
    	Identifier id = new Identifier(match(Token.ID));
    	// 어떤 변수에 입력을 받을 건지 해당 식별자 저장
    	match(Token.SEMICOLON); // read의 끝 확인
    ...
    // <printStmt> -> print <expr>;
       match(Token.PRINT); // 현재 토큰이 print가 맞는지 확인
    // parse print statement
       Expr e = expr(); // 출력할 표현식 저장
       match(Token.SEMICOLON); // print의 끝 확인
       return new Print(e);
    }
...
*/
    ...
    private Expr bexp() {
        // <bexp> -> <aexp> [ (< | <= | > | >= | == | !=) <aexp> ]
        Expr e = aexp();
        if(token.equals(Token.LT) || token.equals(Token.LTEQ) ||
        		token.equals(Token.GT) || token.equals(Token.GTEQ) ||
        		token.equals(Token.EQUAL) || token.equals(Token.NOTEQ)) {
        	Operator op = new Operator(match(token));
        	Expr rExpr = aexp();        	
        	return new Binary(op, e, rExpr);
        } else {
        	return e;
        } // parse relational operations
    }
