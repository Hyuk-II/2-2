...
public class Sint {
    ...
    State free (Decls ds, State state) {
    	...
    	if (ds != null) {
    		for (Decl decl : ds) {
    			...
    			state.pop(); // 지역변수는 가장 최근에 들어있는 값이기 때문에, ds의 길이 만큼 스택에서 값 제거
    		}
    		return state;
    	}
        return null;
    ...
    Value binaryOperation(Operator op, Value v1, Value v2) { // Lab03
        ...
        case "==": // 두 value가 같은지 비교
        	if(v1.type == Type.INT && v2.type == Type.INT) {
        		// 두 값이 INT라면 INT가 같은 값을 가지는 비교
        		return new Value(v1.intValue() == v2.intValue());
        	}
        	else if(v1.type == Type.STRING && v2.type == Type.STRING) {
        		// 두 값이 STRING이라면, 같은 문자열인지 비교
        		// v1의 문자열 값이, v2의 문자열 값과 같은지 비교
            	return new Value(v1.stringValue().equals(v2.stringValue()));
        	}
        	else {
        		// 두 값이 BOOL 이라면, 같은 값인지 비교
        		return new Value(v1.boolValue() == v2.boolValue());
        	}
        	
        case "!=": // 두 value가 다른지 비교
        	if(v1.type == Type.INT && v2.type == Type.INT) {
        		// 두 값이 INT라면 INT가 다른 값을 가지는 비교
        		return new Value(v1.intValue() != v2.intValue());
        	}
        	else if(v1.type == Type.STRING && v2.type == Type.STRING) {
        		// 두 값이 STRING이라면, 같은 문자열인지 비교
        		// v1의 문자열 값이, v2의 문자열 값과 같은지 비교 -> not 연산으로 리턴
            	return new Value(!(v1.stringValue().equals(v2.stringValue())));
        	}
        	else {
        		// 두 값이 BOOL 이라면, 다른 값인지 비교
        		return new Value(v1.boolValue() != v2.boolValue());
        	}
        	
        case "<":
        	// 두 값이 INT라면 INT값을 < 비교연산
        	if(v1.type == Type.INT && v2.type == Type.INT) {
        		return new Value(v1.intValue() < v2.intValue());
        	}
        	// 두 값이 STRING이라면 compareTo()메소드로 비교
        	// 메소드를 실행한 값이 매개변수의 값보다
        	// 작다면(사전순으로 앞에 있다면) 음수, 같으면 0, 크다면(사전순으로 뒤에 있다면)양수
        	else if(v1.type == Type.STRING && v2.type == Type.STRING) {
